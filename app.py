...
# -------------------------------------
# Helpers para calendario y pickup
# -------------------------------------
...
def build_calendar_matrix(df_all: pd.DataFrame, cutoff: pd.Timestamp, start: pd.Timestamp, end: pd.Timestamp, props: Optional[List[str]], mode: str = "Ocupado/Libre") -> pd.DataFrame:
    """Devuelve una matriz (alojamientos x días) con '■' si ocupado o ADR por noche si mode='ADR'."""
    df_cut = df_all[(df_all["Fecha alta"] <= cutoff)].copy()
    if props:
...
# =============================
# MODO 4: Pickup (entre dos cortes)
# =============================
    ...
    if run_pick:
        if pd.to_datetime(cutA) > pd.to_datetime(cutB):
            ...
            plot_df = pd.DataFrame({
                f"A (≤ {pd.to_datetime(cutA).date()})": df_plot[f"{metric_key} A"].values,
                f"B (≤ {pd.to_datetime(cutB).date()})": df_plot[f"{metric_key} B"].values,
            }, index=pd.to_datetime(df_plot["Fecha"]))
            st.line_chart(plot_df, height=300)

            # Tabla detalle por día
            show_cols = ["Fecha", "noches_ocupadas A", "ingresos A", "ocupacion_pct A", "adr A", "revpar A",
                         "noches_ocupadas B", "ingresos B", "ocupacion_pct B", "adr B", "revpar B"]
            st.dataframe(df_plot[show_cols], use_container_width=True)
...
# =============================
# MODO 5: Calendario por alojamiento (heatmap simple)
# =============================
    ...
    if run_cal:
        if pd.to_datetime(cal_start) > pd.to_datetime(cal_end):
            ...
            if piv.empty:
                ...
                # Convertir columnas fecha a string para mostrar días claros en el eje horizontal
